...
if (!class_exists("WPAGLS_Core")) {
	...
	class WPAGLS_Core {
		...
		function __construct() {  
			...
			/* Top */
			add_action( 'wp_head', array($this, 'agls_comment_start') , 1 ); 
 
			 /* Add mandatory agls <meta> elements to the <head> area. */
			add_action( 'wp_head', array($this, 'agls_creator') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_date') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_description') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_title') , 1 );
			add_action( 'wp_head', array($this, 'agls_identifier') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_availability') , 1 );
			add_action( 'wp_head', array($this, 'agls_publisher') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_type') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_function') , 1 );
			add_action( 'wp_head', array($this, 'agls_subject') , 1 );
			
			 /* Add conditional agls <meta> elements to the <head> area. */
			add_action( 'wp_head', array($this, 'agls_audience') , 1 );
			add_action( 'wp_head', array($this, 'agls_coverage') , 1 );
			add_action( 'wp_head', array($this, 'agls_language') , 1 );
			
			 /* Add optional agls <meta> elements to the <head> area. */
			add_action( 'wp_head', array($this, 'agls_contributor') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_format') , 1 );
			add_action( 'wp_head', array($this, 'agls_mandate') , 1 ); 
			add_action( 'wp_head', array($this, 'agls_relation') , 1 );
			add_action( 'wp_head', array($this, 'agls_rights') , 1 );
			add_action( 'wp_head', array($this, 'agls_source') , 1 );
			
			/* Tail */
			add_action( 'wp_head', array($this, 'agls_comment_end'), 1 ); 
		...
		function agls_date() {
			...
			if ( is_singular() ) {
				...
				$tag.= '<meta name="DCTERMS.modified" content="' .  get_the_modified_time( esc_attr__( 'c', hybrid_get_textdomain() ) ) . '"/>' . "\n";
		 ...
		 *
		 */
		function agls_description($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_title($show_default = false, $return = false) {
			...
			if (is_singular() || is_admin() ) {
				...
				$title = __( 'Search', hybrid_get_textdomain() );
			} else if(is_category()) {
				...
				$title = __( 'Archives by date', hybrid_get_textdomain() );
			} else if(is_post_type_archive()) {
				$post_type = get_post_type_object( get_query_var( 'post_type' ) );
				$title = post_type_archive_title("", false);
			} else if(is_archive()) {
				$title = __( 'Archives', hybrid_get_textdomain() );
			} else if(is_404()) {
				$title = __( 'Page Not Found', hybrid_get_textdomain() );
		 ...
		 *
		 */
		function agls_identifier($return = false) {
		 ...
		 *
		 */
		function agls_availability($return = false) {
		 ...
		 *
		 */
		function agls_publisher($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_type($return = false) {
		 ...
		 *
		 */
		function agls_function($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_subject($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_audience($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_coverage($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_language($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_contributor($return = false) {
		 ...
		 *
		 */
		function agls_format($return = false) {
		 ...
		 *
		 */
		function agls_mandate($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_relation($return = false) {
		 ...
		 */
		function agls_rights($show_default = false, $return = false) {
		 ...
		 *
		 */
		function agls_source($return = false) {
